...
- add a full training script that saves model artifacts;
- add stronger frontend input validation (min/max constraints);
- add API tests (`pytest` + `httpx`);
- move model/scaler paths into environment-based config;
...
No license is currently defined in the repository. Add a `LICENSE` file if needed.
